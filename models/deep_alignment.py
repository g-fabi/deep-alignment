...
def IPOT_torch(C, n, m, miu, nu, beta=0.5):
	...
	# miu: bs * n
	device = C.device
	sigma = torch.ones(int(m), 1, device=device, dtype=torch.float) / m # bs * m * 1
	T = torch.ones(n, m, device=device)
...
def IPOT_distance_torch_batch_uniform(C, bs, n, m, iteration=50):
	device = C.device
	C = C.float().to(device)
	T = IPOT_torch_batch_uniform(C, bs, n, m, beta=0.5, iteration=iteration)
	temp = torch.bmm(torch.transpose(C, 1, 2), T)
	distance = batch_trace(temp, m, bs)
	return -distance  # note: our loss later multiplies by (-1)
...
def IPOT_torch_batch_uniform(C, bs, n, m, beta=0.5, iteration=50):
	device = C.device
	sigma = torch.ones(bs, int(m), 1, device=device) / float(m)
	T = torch.ones(bs, n, m, device=device)
...
def batch_trace(input_matrix, n, bs):
	...
	a = torch.eye(n, device=device).unsqueeze(0).repeat(bs, 1, 1)
	b = a * input_matrix
	return torch.sum(torch.sum(b, -1), -1).unsqueeze(1)

class DeepAlignmentLoss(torch.nn.Module):
    """
    Deep Alignment Loss computes an optimal transport (OT)
    distance between the local tokens of two modalities.
    It expects:
      • spatial_a: [B, token_count1, d] (e.g. skeleton joints)
      • spatial_b: [B, token_count2, d] (e.g. imu channels)
      • temporal_a: [B, T1, d]
      • temporal_b: [B, T2, d]
    Uniform weights are assumed. Internally, it computes the cosine distance
    and then applies the IPOT algorithm (with uniform weights) to produce an OT cost.
    The final loss is a (weighted) sum over spatial and temporal alignments.
    """
    def __init__(self, weight_spatial=1.0, weight_temporal=1.0, beta=0.5, iteration=50, common_dim=512):
        super(DeepAlignmentLoss, self).__init__()
        ...
        self.proj_spatial_a = DAProjectionLayer(common_dim)
        self.proj_spatial_b = DAProjectionLayer(common_dim)
        self.proj_temporal_a = DAProjectionLayer(common_dim)
        self.proj_temporal_b = DAProjectionLayer(common_dim)

    def batch_cosine_distance(self, a, b):
        # a: [B, n, d], b: [B, m, d]
        a_norm = a / (torch.norm(a, p=2, dim=2, keepdim=True) + 1e-12)
        b_norm = b / (torch.norm(b, p=2, dim=2, keepdim=True) + 1e-12)
        sim = torch.bmm(a_norm, b_norm.transpose(1, 2))  # [B, n, m]
        return 1 - sim

    def forward(self, spatial_a, spatial_b, temporal_a, temporal_b):
        bs = spatial_a.size(0)
        # Project tokens to the common dimension:
        spatial_a = self.proj_spatial_a(spatial_a)
        spatial_b = self.proj_spatial_b(spatial_b)
        temporal_a = self.proj_temporal_a(temporal_a)
        temporal_b = self.proj_temporal_b(temporal_b)

        # Compute spatial alignment loss.
        n1 = spatial_a.size(1)
        n2 = spatial_b.size(1)
        cost_spatial = self.batch_cosine_distance(spatial_a, spatial_b)  # [B, n1, n2]
        loss_spatial = -IPOT_distance_torch_batch_uniform(cost_spatial, bs, n1, n2, iteration=self.iteration)
        loss_spatial = loss_spatial.mean()

        # Compute temporal alignment loss.
        t1 = temporal_a.size(1)
        t2 = temporal_b.size(1)
        cost_temporal = self.batch_cosine_distance(temporal_a, temporal_b)  # [B, t1, t2]
        loss_temporal = -IPOT_distance_torch_batch_uniform(cost_temporal, bs, t1, t2, iteration=self.iteration)
        loss_temporal = loss_temporal.mean()

        total_loss = self.weight_spatial * loss_spatial + self.weight_temporal * loss_temporal
        return total_loss, loss_spatial, loss_temporal

class DeepAlignmentModel(LightningModule):
    def __init__(self, modalities, encoders, loss_fn, optimizer_name='adam', lr=0.001, **kwargs):
        super().__init__()
    ...
    def _forward_one_modality(self, modality, x):
        ...
        if isinstance(out, (tuple, list)):
            if len(out) >= 3:
                ...
                raise ValueError("Encoder output format not supported for DeepAlignmentModel")
        ...
        if spatial.dim() == 3 and spatial.shape[1] < spatial.shape[2]:
            spatial = spatial.transpose(1, 2)
        if temporal.dim() == 3 and temporal.shape[1] < temporal.shape[2]:
    ...
    def forward(self, batch):
        ...
        for m in self.modalities:
            outputs[m] = self._forward_one_modality(m, batch[m])
        return outputs
    ...
    def training_step(self, batch, batch_idx):
        ...
        for m in self.modalities:
            outputs[m] = self._forward_one_modality(m, batch[m])
        spatial_a = outputs[self.modalities[0]]['spatial']
        spatial_b = outputs[self.modalities[1]]['spatial']
        temporal_a = outputs[self.modalities[0]]['temporal']
        temporal_b = outputs[self.modalities[1]]['temporal']
        loss, loss_spatial, loss_temporal = self.loss_fn(spatial_a, spatial_b, temporal_a, temporal_b)
        self.log("da_train_loss", loss)
